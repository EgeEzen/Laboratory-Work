#RNA-SEQ Workflow for homo sapiens (paired-end reads)
...
samples_pairone=$location/SRR/*R1_001.fastq.gz
...
samples_pairtwo=($location/SRR/*R2_001.fastq.gz)
...
do
	annot=${sample_annot[$i]} #now the annotation is in annot variable
        ...
        hisat2 -p 10 --dta -x $location/grch38_tran/genome_tran -1 $sample -2${samples_pairtwo[$i]} --summary-file $location/SRR/$annot.txt -S $location/sam/$annot.sam
